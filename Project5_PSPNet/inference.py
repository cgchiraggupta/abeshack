...
class PSPNetInference:
    def __init__(self, model_path, device='cuda'):
        self.device = torch.device(device if torch.cuda.is_available() else 'cpu')
        self.model = PSPNet(num_classes=11, backbone='resnet101', pretrained=False).to(self.device)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='PSPNet Inference for Off-Road Terrain Segmentation')
    parser.add_argument('--model', type=str, default='pspnet_final.pth',
    ...
    inference = PSPNetInference(args.model)
