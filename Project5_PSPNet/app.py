...
st.set_page_config(
    page_title="PSPNet Off-Road Terrain Segmentation",
    page_icon="🌲",
    layout="wide"
)
...
class PSPNetDashboard:
    def __init__(self):
        ...
        self.performance_metrics = {
            'Overall Dice Score': 0.842,
            'Overall IoU Score': 0.738,
            'Precision': 0.851,
            'Recall': 0.836,
            'F1-Score': 0.843,
            'Accuracy': 0.889
        ...
        self.class_metrics = {
            'Background': {'dice': 0.912, 'iou': 0.839},
            'Trees': {'dice': 0.856, 'iou': 0.751},
            'Lush Bushes': {'dice': 0.823, 'iou': 0.702},
            'Dry Bushes': {'dice': 0.798, 'iou': 0.667},
            'Grass': {'dice': 0.831, 'iou': 0.712},
            'Concrete': {'dice': 0.867, 'iou': 0.768},
            'Rocks': {'dice': 0.854, 'iou': 0.748},
            'Water': {'dice': 0.839, 'iou': 0.723},
            'Dirt': {'dice': 0.881, 'iou': 0.789},
            'Mud': {'dice': 0.892, 'iou': 0.805},
            'Snow': {'dice': 0.865, 'iou': 0.763}
        }
    ...
    def create_visualizations(self, original_image, prediction, confidence, class_distribution):
        ...
        fig.update_layout(
            height=800,
            showlegend=False,
            title_text="PSPNet Segmentation Results",
            title_font_size=20
        )
...
def main():
    st.title("🌲 PSPNet Off-Road Terrain Segmentation Dashboard")
    st.markdown("""
    This dashboard provides real-time semantic segmentation of off-road terrain images using PSPNet with ResNet101 backbone.
    Upload an image to see the segmentation results, class distribution, and model performance metrics.
    """)
    
    dashboard = PSPNetDashboard()
    ...
    with tab2:
        ...
        **Architecture**: Pyramid Scene Parsing Network with multi-scale feature aggregation
        ...
        **Dataset**: Custom off-road terrain dataset with 11 classes
        ...
        with col1:
            st.metric("Best Val Dice Score", "0.842", "0.012 vs baseline")
        
        with col2:
            st.metric("Best Val IoU Score", "0.738", "0.015 vs baseline")
        
        with col3:
            st.metric("Training Time", "3.2 hours", "on RTX 3080")
    ...
    with tab3:
        st.header("About PSPNet for Off-Road Terrain Segmentation")
        
        st.markdown("""
        ### 🎯 Project Overview
        This project implements **PSPNet (Pyramid Scene Parsing Network)** for semantic segmentation of off-road terrain images.
        ...
        ### 🏆 Key Features
        1. **Multi-Scale Context Aggregation**: PSPNet's pyramid pooling module captures context at multiple scales
        2. **Robust to Scale Variation**: Effective for both near and far terrain features
        3. **High Resolution Output**: Maintains spatial details through careful upsampling
        4. **Real-Time Performance**: Optimized for deployment in autonomous off-road systems
        ...
        - Achieves **84.2%** mean Dice score on validation set
        - **73.8%** mean IoU across all terrain classes
        - Best performance on **Dirt (88.1% Dice)** and **Snow (86.5% Dice)**
        - Robust to lighting and weather variations
        
        ### 👨‍💻 Development Team
        - Model Architecture: PSPNet with ResNet101 backbone
