...
class SegDataset(Dataset):
    def __init__(self, img_paths, mask_paths, augment=True):
        ...
        # Exact class mapping from hackathon instructions
        self.class_map = {
            100: 0,   # Trees
            200: 1,   # Lush Bushes
            300: 2,   # Dry Grass
        ...
        # Stronger augmentation pipeline for better generalization
        self.transform = A.Compose([
            ...
            A.Resize(512, 512), # Required for consistent batch sizing
        ])
    ...
    def remap_mask(self, mask):
        remapped = np.zeros_like(mask, dtype=np.int64) + 8 # Default to Landscape if unknown
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            # Resize for validation if not augmenting
            val_transform = A.Resize(512, 512)
            augmented = val_transform(image=img, mask=mask)
            img, mask = augmented['image'], augmented['mask']

        # Preprocessing: Normalize and conversion to Tensor format
