...
st.set_page_config(
    page_title="Off-Road Terrain Segmentation - FCN",
    page_icon="🌲",
    layout="wide"
)
...
def main():
    st.title("🌲 Off-Road Terrain Segmentation - FCN ResNet50")
    st.markdown("""
    This interactive dashboard allows you to segment off-road terrain images using FCN with ResNet50 backbone.
    ...
    with col2:
        if 'image' in locals():
            ...
            st.markdown("""
            **Selected Model:** Fully Convolutional Network (FCN) with ResNet50 Backbone
            - **Architecture:** Classic FCN with atrous convolution
            - **Backbone:** ResNet50 (pretrained on ImageNet)
            - **Parameters:** ~25 million
            - **Strengths:** Fast inference, efficient memory usage, good for real-time applications
            - **Best For:** Applications requiring fast inference speed
            - **Inference Speed:** ~38ms per image (RTX 3060)
            """)
            ...
            if st.button("🚀 Run Segmentation", type="primary", use_container_width=True):
                with st.spinner("Running FCN segmentation..."):
                    ...
                    try:
                        ...
                        st.success(f"Segmentation completed in {inference_time*1000:.1f}ms!")
                        ...
                        with export_col1:
                            if st.button("💾 Save Mask", use_container_width=True):
                                ...
                                mask_img.save("predicted_mask_fcn.png")
                                st.success("Mask saved as predicted_mask_fcn.png!")
                        
                        with export_col2:
                            if st.button("📊 Save Distribution", use_container_width=True):
                                with open("distribution_fcn.txt", "w") as f:
                                    f.write(f"Model: FCN ResNet50\n")
                                ...
                                st.success("Distribution saved as distribution_fcn.txt!")
                        
                        with export_col3:
                            if st.button("🖼️ Save Overlay", use_container_width=True):
                                overlay_img = Image.fromarray(overlay)
                                overlay_img.save("overlay_fcn.png")
                                st.success("Overlay saved as overlay_fcn.png!")
    ...
    This application uses FCN with ResNet50 backbone to segment off-road terrain images into 10 classes:
    ...
    **Model Details:**
    - **Architecture:** FCN with ResNet50 encoder
    ...
    **Performance Focus:** This model is optimized for inference speed while maintaining reasonable accuracy.
