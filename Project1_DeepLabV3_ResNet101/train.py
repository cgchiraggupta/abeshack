...
def train():
    # Parameters
    NUM_CLASSES = 10  # 0-9 for the 10 mapped classes
    
    EPOCHS = 40  # Increased for recovery
    BATCH_SIZE = 4
    LEARNING_RATE = 1e-4
    ...
    scheduler = ReduceLROnPlateau(
        optimizer,
        mode="max",          # because Dice should increase
    ...
    # Combined Loss
    # Combined Loss (Updated with computed weights)
    # Weights from compute_weights.py:
    # [50.4137, 50.4162, 2.9984, 50.4182, 50.418, 50.4183, 50.4183, 50.418, 16.598, 3.2536]
    class_weights = torch.tensor([
        50.4137, 50.4162, 2.9984, 50.4182, 50.418, 50.4183, 50.4183, 50.418, 16.598, 3.2536
    ]).to(DEVICE)
    # Note: CombinedLoss inside losses.py needs to accept 'weight' for CrossEntropy
    # We passed it in __init__
    ...
    for epoch in range(EPOCHS):
        # Encoder Freezing Logic (First 5 epochs)
        if epoch < 5:
        ...
        for imgs, masks in pbar:
            ...
            # Metric logging
            # Metric logging
            # dice_score now returns (mean, per_class)
        ...
        # Update Scheduler & Early Stopping based on VAL DICE
