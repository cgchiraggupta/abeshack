...
def evaluate_model():
    ...
    print(f"Evaluating UNet (ResNet50) on {len(val_imgs)} validation images")
    ...
    print(f"\n{'='*50}")
    print(f"Evaluation Results for UNet (ResNet50)")
    print(f"{'='*50}")
    ...
    plt.title('Confusion Matrix - UNet (ResNet50)')
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.tight_layout()
    plt.savefig('confusion_matrix_unet.png', dpi=300)
    plt.close()
    
    print(f"\nConfusion matrix saved to confusion_matrix_unet.png")
