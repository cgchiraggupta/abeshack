...
st.set_page_config(
    page_title="Off-Road Terrain Segmentation - UNet",
    page_icon="🌲",
    layout="wide"
)
...
def main():
    st.title("🌲 Off-Road Terrain Segmentation - UNet (ResNet50)")
    st.markdown("""
    This interactive dashboard allows you to segment off-road terrain images using UNet with ResNet50 backbone.
    ...
    with col2:
        if 'image' in locals():
            ...
            st.markdown("""
            **Selected Model:** UNet with ResNet50 Backbone
            - **Architecture:** Classic encoder-decoder with skip connections
            - **Encoder:** ResNet50 (pretrained on ImageNet)
            - **Parameters:** ~31 million
            - **Strengths:** Excellent for capturing fine details, good balance of speed and accuracy
            - **Best For:** Detailed terrain features and complex off-road scenes
            """)
                    ...
                    try:
                        ...
                        with export_col1:
                            if st.button("💾 Save Mask", use_container_width=True):
                                ...
                                mask_img.save("predicted_mask_unet.png")
                                st.success("Mask saved as predicted_mask_unet.png!")
                        
                        with export_col2:
                            if st.button("📊 Save Distribution", use_container_width=True):
                                with open("distribution_unet.txt", "w") as f:
                                    f.write(f"Model: UNet (ResNet50)\n")
                                ...
                                st.success("Distribution saved as distribution_unet.txt!")
                        
                        with export_col3:
                            if st.button("🖼️ Save Overlay", use_container_width=True):
                                overlay_img = Image.fromarray(overlay)
                                overlay_img.save("overlay_unet.png")
                                st.success("Overlay saved as overlay_unet.png!")
    ...
    This application uses UNet with ResNet50 backbone to segment off-road terrain images into 10 classes:
    ...
    **Model Details:**
    - **Architecture:** UNet with ResNet50 encoder
