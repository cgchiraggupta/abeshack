...
# Page configuration
st.set_page_config(
    page_title="Off-Road Terrain Segmentation - DeepLabV3+ ResNet50",
    page_icon="🌲",
    layout="wide"
)
...
st.markdown('<h3 style="text-align: center; color: #666;">DeepLabV3+ ResNet50 Model Dashboard</h3>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("### About")
    st.info("""
    This dashboard uses DeepLabV3+ with ResNet50 backbone for semantic segmentation of off-road terrain.
...
if page == "🏠 Home":
    ...
    This application uses **DeepLabV3+ with ResNet50 backbone** for semantic segmentation of off-road environments.
    ...
    2. Check **"Model Info"** for technical details about DeepLabV3+
    3. View **"Performance"** for model evaluation metrics
    4. Try **"Video Analysis"** for processing video footage
    
    ### Model Architecture:
    
    **DeepLabV3+** is a state-of-the-art semantic segmentation model that uses atrous spatial pyramid pooling (ASPP)
    to capture multi-scale contextual information. Combined with ResNet50 backbone, it provides excellent
    accuracy with reasonable computational requirements.
    """)
    ...
    with col1:
        st.metric("Model Parameters", "59M")
    ...
    with col4:
        st.metric("Best Dice Score", "0.835")

# Model Info Page
elif page == "📊 Model Info":
    st.markdown('<h2 class="sub-header">📊 DeepLabV3+ ResNet50 Model Architecture</h2>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        **DeepLabV3+ with ResNet50 backbone**:
        ...
        2. **ASPP Module**: Multi-scale feature extraction
        3. **Encoder-Decoder**: Improved boundary segmentation
        4. **ResNet50 Backbone**: Strong feature extraction capabilities
    ...
    with col2:
        ...
        st.image("https://production-media.paperswithcode.com/methods/Screen_Shot_2020-07-07_at_2.25.01_PM.png", 
    ...
    performance_data = {
        'Model': ['DeepLabV3+ ResNet50', 'EfficientNet-B4', 'UNet', 'PSPNet', 'FCN'],
        'Dice Score': [0.835, 0.842, 0.828, 0.821, 0.815],
        'IoU Score': [0.782, 0.789, 0.775, 0.768, 0.761],
        'Params (M)': [59, 19, 31, 49, 35]
    }
        ...
        if model and st.button("🚀 Run Segmentation", use_container_width=True):
            with st.spinner("Processing image..."):
                ...
                with col1:
                    ...
                    mask_bytes = cv2.imencode('.png', cv2.cvtColor(colored_mask_resized, cv2.COLOR_RGB2BGR))[1].tobytes()
                    st.download_button(
                        label="Download Mask",
                        data=mask_bytes,
...
# Performance Page
    ...
    with col1:
        st.metric("Best Dice Score", "0.835", "±0.012")
    
    with col2:
        st.metric("Best IoU Score", "0.782", "±0.015")
    
    with col3:
        st.metric("Training Time", "5.8h", "-")
    
    with col4:
        st.metric("Inference Speed", "68ms", "per image")
...
<div style="text-align: center; color: #666; font-size: 0.9rem;">
    <p>Off-Road Terrain Semantic Segmentation Dashboard • DeepLabV3+ ResNet50 Model • Version 1.0</p>
