...
def main():
    ...
    final_model = UNetPlusPlus(num_classes=11, deep_supervision=False).to(device)
    checkpoint = torch.load('best_model.pth', map_location=device)
    final_model.load_state_dict(checkpoint['model_state_dict'])
    torch.save(final_model.state_dict(), 'unetplusplus_final.pth')
    print("Final model saved as unetplusplus_final.pth")
