...
def test_model(test_dir="data/testImages", has_masks=False):
    ...
    print("Testing FCN ResNet50 model")
    print("="*60)
    ...
    if has_masks:
        ...
        plt.title('Test Confusion Matrix - FCN ResNet50')
        plt.xlabel('Predicted')
        plt.ylabel('True')
        plt.tight_layout()
        plt.savefig('test_confusion_matrix_fcn.png', dpi=300)
        plt.close()
        
        print(f"\nConfusion matrix saved to test_confusion_matrix_fcn.png")
        ...
        with open('test_report_fcn.txt', 'w') as f:
            f.write("Test Report - FCN ResNet50\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"Detailed report saved to test_report_fcn.txt")
        ...
        for img_path in tqdm(test_images, desc="Processing test images"):
            try:
                ...
                plt.suptitle(f"FCN ResNet50 - {os.path.basename(img_path)}", fontsize=14)
                plt.tight_layout()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test FCN model')
